...
func GetScheduledMaintenancesFactoryController(context *gin.Context) {
	maintenanceRepository := database.NewPgMaintenanceRepository()

	getScheduledMaintenancesUseCase := maintenanceusecases.NewGetScheduledMaintenancesUseCase(maintenanceRepository)

	maintenancecontroller.GetScheduledMaintenancesController(context, getScheduledMaintenancesUseCase)
