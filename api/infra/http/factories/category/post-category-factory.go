...
func PostCategoryControllerFactory(context *gin.Context) {
	categoryRepository := database.NewPGCategoryRepository()

	categoryUseCase := categoryusecases.NewPostCategoryUseCase(categoryRepository)

	categorycontroller.PostCategoryController(context, categoryUseCase)
}
